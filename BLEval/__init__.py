...
from BLEval.computeFDR import FDR
from BLEval.computeUndirectedFDR import UndirectedFDR
...
class BLEval(object):
    ...
    def computeUndirectedFDR(self):

        '''
        For each algorithm-dataset combination,
        this function computes the actual/empirical FDR at 
        various nominal/targeted FDRs. It is computed
        without reference to the direction of each edge.
        Target --> regulator is considered "correct".
        
        :returns: 
            A dataframe containing the FDR values 
            for each algorithm-dataset combination.
        ...
        for algo in tqdm(self.input_settings.algorithms, unit = " Algorithms"):
            if algo[1]['should_run'] == True:
                X[algo[0]] = UndirectedFDR(self, algo[0])
        X = pd.DataFrame(X)
        better_format = {}
        for algo in tqdm(self.input_settings.algorithms, unit = " Algorithms"):
            if algo[1]['should_run'] == True:
                better_format[algo[0]] = X[algo[0]].apply(pd.Series)
                better_format[algo[0]]["method"] = algo[0]
        better_format = pd.concat(better_format.values())
